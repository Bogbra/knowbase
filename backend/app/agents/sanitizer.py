"""User content sanitization — heuristic defense-in-depth against prompt injection.

This is NOT the security boundary. Keyword/pattern matching can always be
worded around. The actual boundary is that user input and retrieved chunks
are placed in clearly delimited context and never concatenated into the
system prompt as instructions — the LLM is told what is data vs. directive
by structure, not by scanning for known-bad phrases. This module only makes
common, unsophisticated injection attempts visible as literal text instead
of silently doing what they say.
"""
